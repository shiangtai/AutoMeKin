...
! loop over the atoms
   do i=1,natom
rij:  do j=1,natom
         if(i==1) read(*,*) q(3*j-2,jstep),q(3*j-1,jstep),q(3*j,jstep)
!c rij at every step for atom i
...
!         print "(t3,6(i7))", jstep,i,jold(i),jnew(i),nrxn(i,jold(i),jnew(i)),tnm(i,jold(i),jnew(i))
...
! uncomment for debugging
!            print "(t3,a,i4)","Total number of bonds",sum(nm)/2
!            do idum=1,natom
!               print "(t3,10(i4))",idum,(nm(idum,j),j=1,natom)
!            enddo
! re-set variables 
            nrxn(i,:,:)=0
! calculate the code of the new structure
            ...
            do iii=1,natom
               ...
               enddo
            enddo
! when the number of bonds is less than the initial value stop
!            if(sum(nm)<tnb0) jfinish=jstep 
         endif
      endif
! 
   enddo 
...
npc:   do ijk=1,npathc
      ...
      ijc=0
!s1:   do j=1,ijk-1 
!         prod1=code(ijk)*code(j)-code(ijk-1)*code(j-1)
!         prod2=code(ijk)*code(j-1)-code(ijk-1)*code(j)
!         if(prod1==0.or.prod2==0 ) then
!            print "(t3,a,i5,a)","Path number",ijk," is repeated"
!            cycle npc 
!         endif
!      enddo s1
...
nb3:  do kk=1,natom
      ...
      do i=1,natom
         ...
         do j=1,irange
            ...
            if(idlt<1) idlt=1
            iunit=100*ijk+j
            write(iunit,6) q(3*i-2,idlt),nmmb,q(3*i-1,idlt),nmmb,q(3*i,idlt),nmmb
         enddo 
      enddo
   enddo npc
endif
print "(t3,a,/)","*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*"
6 format(3(f20.10,i5))
!
end program  bbfs
