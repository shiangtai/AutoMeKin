...
###
#     awk '{if(NR==1) natom=$1};END{print NR-natom-2}' $tsdirhl/MINs/SORTED/MIN$min1"_"*.rxyz >> $tsdirhl/KMC/RRKM/"proc1_TS"$procn".dat"
#     awk '{if(NR>1 && NF==1) printf "%10.0f\n",sqrt($1*$1)}' $tsdirhl/MINs/SORTED/MIN$min1"_"*.rxyz  >> $tsdirhl/KMC/RRKM/"proc1_TS"$procn".dat"
#     awk '{if(NR==1) natom=$1};END{print NR-natom-3}' $tsdirhl/MINs/SORTED/MIN$min1"_"*.rxyz >> $tsdirhl/KMC/RRKM/"proc1_TS"$procn".dat"
#     awk '{if(NR>1 && NF==1 && $1>0) printf "%10.0f\n",$1}' $tsdirhl/TSs/$ts >> $tsdirhl/KMC/RRKM/"proc1_TS"$procn".dat"
  ...
  if [ $nproc -eq 2 ]; then
     ...
     if [ $rate -eq 0 ]; then
        ...
        deltag="$(echo "$g1" "$g2" | awk '{printf "%10.3f",$1-$2}')"
#        awk '{if(NR==2) printf "%14.9f\n",$2+$6}' $tsdirhl/TSs/$ts  > tmp_dumi
#        awk '{if(NR==2) printf "%14.9f\n",$2+$6}' $tsdirhl/MINs/SORTED/MIN$min2"_"*.rxyz >> tmp_dumi
#        deltag=`awk '{e[NR]=$1};END{printf "%10.3f",(e[1]-e[2])*627.51}' tmp_dumi`
...
###
#        awk '{if(NR==1) natom=$1};END{print NR-natom-2}' $tsdirhl/MINs/SORTED/MIN$min2"_"*.rxyz >> $tsdirhl/KMC/RRKM/"proc2_TS"$procn".dat"
#        awk '{if(NR>1 && NF==1) printf "%10.0f\n",sqrt($1*$1)}' $tsdirhl/MINs/SORTED/MIN$min2"_"*.rxyz  >> $tsdirhl/KMC/RRKM/"proc2_TS"$procn".dat"
#        awk '{if(NR==1) natom=$1};END{print NR-natom-3}' $tsdirhl/MINs/SORTED/MIN$min2"_"*.rxyz >> $tsdirhl/KMC/RRKM/"proc2_TS"$procn".dat"
#        awk '{if(NR>1 && NF==1 && $1>0) printf "%10.0f\n",$1}' $tsdirhl/TSs/$ts >> $tsdirhl/KMC/RRKM/"proc2_TS"$procn".dat"
...
set `awk '{print $1}' $tsdirhl/KMC/branching$postb `
for i
do
...
#      prod=`FormulaPROD.sh $tsdirhl/PRODs/PR$code"_"*`
