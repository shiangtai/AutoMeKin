...
eref=`awk '{if($2=='$imin') print $4}' $tsdirhl/MINs/SORTED/MINlist_sorted`
#echo $eref
...
#      rm -f tmp_calc
###
#      sqlite3 $tsdirhl/PRODs/CALC/prodfhl.db "select energy,zpe from prodfhl where name='$namesql'" | sed 's@|@ @g' > tmp_calc
#      sqlite3 $tsdirhl/MINs/SORTED/minshl.db "select energy,zpe from minshl where name='$namemsql'" | sed 's@|@ @g' >> tmp_calc
#      emin2=`awk '{e[NR]=$1;ezpe[NR]=$2};END{print (e[1]-e[2])*627.51+ezpe[1]-ezpe[2]}' tmp_calc`
...
#      pr=`FormulaPROD.sh $tsdirhl/PRODs/CALC/PR$code"_"*.rxyz`
