...
if [ ! -d "$tsdirhl/KMC" ]; then
   echo "$tsdirhl/KMC does not exist. It will be created"
   mkdir $tsdirhl/KMC
else
   echo "$tsdirhl/KMC already exists."
fi
...
#   for i in $(ls $tsdirhl/MINs/SORTED/MIN*.rxyz)
   do 
#      name="$(basename $i)"
      echo $name
      echo "Labels" >labels
      sqlite3 ${tsdirhl}/MINs/SORTED/minshl.db "select geom from minshl where lname='$name'"  >mingeom
...
#sort conf_isomer.out file
   ...
   echo "conf_isomers already calculated and possibly changed. They will not be re-calculated again"
fi
...
#set `awk '{if(NR>2) print $3}' $tsdirhl/KMC/RXNet.cg `
#for i
...
# symm.sh $tsdirhl/TSs/$i
 ...
 symm.sh tmp_inp
# symm.sh $tsdirhl/MINs/SORTED/MIN$nmin1"_"*
 ...
 if [ $nmin2 -gt 0 ]; then
   ...
   symm.sh tmp_inp
#   symm.sh $tsdirhl/MINs/SORTED/MIN$nmin2"_"*
...
echo "Checking $tsdirhl/KMC/RXNet_long.cg file"
compare_minfailed_with_other_minima_hl.sh
