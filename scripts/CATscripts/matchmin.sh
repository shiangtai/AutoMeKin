file=$1
path1=`awk '/From=/{print "tsdirHL_"$2"/PRODs"}' $file` 
path2=`awk '/Where=/{print "tsdirHL_"$2}' $file` 
formul=`awk 'NR==3,NR==3{print $1}' $file`
number=`awk 'NR==4,NR==4{print $1}' $file`
HLcalc=`awk 'NR==5,NR==5{print $1}' $file`
noHLcalc=`awk 'NR==5,NR==5{print $2}' $file`
sed 's/\./ /g' $path1/PRlist_frag >dumi
log=`awk '{if($2=='$number' && $5=="'$formul'") print $5"."$6"."$7".log" 
if($2=='$number' && $9=="'$formul'") print $9"."$10"."$11".log"}' dumi`
awk '{if(NR>2) {
	j=0
	p=1
	while(j<=i){
...
energy=`get_energy$HLcalc.sh $path1/CALC/$log $noHLcalc`
